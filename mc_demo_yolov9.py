...
from yolov9.utils.dataloaders import LoadStreams, LoadImages
from yolov9.utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression, \
    apply_classifier, \
    xyxy2xywh, strip_optimizer, set_logging, increment_path ,scale_boxes #scale_coords
...
# from models.pytorch_utils import Traced
# from tracker.tracking_utils.timer import Timer

# from tracker.mc_SMILEtrack import SMILEtrack
# from tracker.mc_bot_sort import BoTSORT
from tapnet_object_tracking import SMPT
# from clip_object_tracking import CLIP_SIMILE
# from tracker.tracking_utils.timer import Timer
...
def detect(save_img=True):
    ...
    # tracker = CLIP_SIMILE(opt, device, frame_rate=30.0)
    tracker = SMPT(opt, device, frame_rate=30.0)
    ...
    for path, img, im0s, vid_cap,_ in tqdm(dataset, desc=f'tracking {opt.name}'):
        frameID += 1
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            detections = []
            if len(det):
                # boxes = det[:, :4] #scale_coords(img.shape[2:], det[:, :4], im0.shape)
                ...
                if tlwh[2] * tlwh[3] > opt.min_box_area:
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if 'car' in label: # AICUP only have one cls: car
                            ...
                            annotator.box_label(tlbr, label=label, color=colors[int(tid) % len(colors)])
            p = Path(p)  # to Path
    ...
    with open("/submision/v9_C_E_3/"+  f"{opt.name}.txt", 'w') as f:
        f.writelines(results)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='yolov9.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--project', default='runs/TAPNET_TEST_CLIP_SIMILE', help='save results to project/name')
