        ...
        if self.frame_id == 1:
          ...
          for detection in detections:
              ...
              for i, last_detection in enumerate(self.last_detection):
                  ...
                  new_points = self.generate_points(xyxy, 6, 6)  # Assuming generate_points adequately distributes points within bbox
                  new_points = self.convert_coordinates(new_points, 1280, 720, 512, 512).cpu()
                  query_points = self.convert_select_points_to_query_points(0, new_points)  # Convert generated points for querying
                  
                  # Assuming image_concat and querying are setup correctly
                  image_concat = torch.tensor(np.array(self.img_connect)).to(self.device)
                  query_points = torch.tensor(query_points).to(self.device)
                  tracks, visibles = inference(image_concat, query_points, self.model_track)  # Get tracks and visibility
                  tracks=np.transpose(tracks.cpu(),(1,0,2))
                  visibles=np.transpose(visibles.cpu(),(1,0))
                  track_points = tracks[1,:]
                  track_points = self.convert_coordinates(track_points, 512, 512, 1280, 720)
                  track_points=track_points[visibles[1]==True]
                  match_point = self.check_points_in_bbox(track_points, current_bbox)
                  point_count.append(match_point)
              object_point_score.append(point_count)
          
          remember_dict = {}
          lost_ids = []
          for ind in range(len(self.last_detection)):
            #點追蹤
            max_index = max((index for index, score in enumerate(object_point_score) if score[ind] >= self.threshold), key=lambda index: object_point_score[index][ind], default=None)
            ...
            if max_index != None and detections[max_index].track_id != 0:
                ...
                if last_information != "Key not found":
                    ...
                    match_xyxy = [self.tlwh_to_xyxy(self.last_detection[ind].tlwh), self.tlwh_to_xyxy(detections[max_index].tlwh)]
                    r_match = self.extract_image_patches(self.save_image, match_xyxy)
                    sim_score = self.generateScore(r_match)
                    if sim_score > last_conf:
            ...
            if max_index != None and detections[max_index].track_id == 0:
                ...
                r_match = self.extract_image_patches(self.save_image, match_xyxy)
                sim_score = self.generateScore(r_match)
                remember_dict[str(max_index)] = [sim_score, ind]
                if sim_score >= iou:
                    detections[max_index].track_id = self.last_detection[ind].track_id
                else:
                    lost_ids.append(ind)
          ...
          for lost_id in lost_ids:
            ...
            for ind ,detection in enumerate(detections):
                if detection.track_id == 0:
                    #相似度
                    match_xyxy = [self.tlwh_to_xyxy(self.last_detection[lost_id].tlwh), self.tlwh_to_xyxy(detection.tlwh)]
                    iou = self.compute_iou(match_xyxy[0], match_xyxy[1]) *0.15 + 0.6
                    r_match = self.extract_image_patches(self.save_image, match_xyxy)
                    sim_score = self.generateScore(r_match)
                    if sim_score >= iou:
                        ind_list.append(ind)
                        similar.append(sim_score)
                    # else:
                    #     print("sim_score:",sim_score)
                    #     detection.track_id = detection.next_id()
            all_ind_list.append(ind_list)
            object_similar_score.append(similar)
            # print("all_ind_list:", all_ind_list)
            # print("object_similar_score:", object_similar_score)
          
          for i, (obj_sim, ind_list) in enumerate(zip(object_similar_score, all_ind_list)):
            if obj_sim != []:
                max_index = max(range(len(obj_sim)), key=obj_sim.__getitem__)
                now_obj = ind_list[max_index]
                before_obj = lost_ids[i]
                last_score = remember_dict.get(str(now_obj), "Key not found")
                if last_score != "Key not found":
                    if obj_sim[max_index] > last_score:
                        detections[now_obj].track_id = self.last_detection[before_obj].track_id
                        remember_dict[str(now_obj)] = obj_sim[max_index]
                else:
                    detections[now_obj].track_id = self.last_detection[before_obj].track_id
                    remember_dict[str(now_obj)] = obj_sim[max_index]
